...
function getResult() {
	...
	var geoUrl =
		...
		',' +
		countryCode +
		'&limit=5&appid=7d1b285353ccacd5326159e04cfab063';

	//pass the requestUrl variable as an argument to the fetch() method
	fetch(geoUrl)
		//convert the response into JSON, then return the JSON-formatted response
		...
		.then(function (data) {
			...
			var weatherUrl =
				...
				'&lon=' +
				geoLon +
				'&exclude=minutely,hourly,alerts&units=imperial&appid=7d1b285353ccacd5326159e04cfab063';
			...
			fetch(weatherUrl)
				...
				.then(function (data) {
					...
					//display the fetched 5 day weather data
					for (var i = 1; i < 6; i++) {
...
//add search history data to localstorage
function addInfo(n) {
